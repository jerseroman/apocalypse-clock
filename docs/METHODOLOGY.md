...
This document describes Apocalypse Clock model 1.3.1-dev with dataset 1.9.1. The initial 1.3 development baseline was a functional copy of model 1.2.9. The current revision changes the active causal topology. It is an experimental scenario model, not a peer-reviewed or empirically validated forecast. Numerical conventions below describe the implementation; they are not measurements of collapse probability.

## Inputs and implementation

The dataset [data/data_v1_9_1_2026-09-23.json](../data/data_v1_9_1_2026-09-23.json) contains 23 headline threats and eight flat metric entries per headline threat: scale, urgency, acceleration, interdependence, irreversibility, governance failure, growth rate and threshold. The 184-entry contract does not include metadata. It also represents 36 nested Climate Breakdown and Ocean Degradation subsystem components in `_meta.subsystem_models`, for 59 modeled risk elements in total. The nested components expose their own timelines and literature/model horizons but are not treated as 36 additional independent headline threats.

Functional definitions, fixed criticality weights, service memberships and overlap groups are read from the dataset's _meta.functional_model.nodes and mirrored threshold entries. The active topology, event gates, edge weights, lags and reviewed mechanism labels are defined in [src/causal-graph.js](../src/causal-graph.js) and shared by scoring, cascade calculations, network statistics and visualization. Dataset 1.9.1 contains dependency records as documented metadata; those fields are **not active graph inputs**, including when a source map is uploaded. Growth conventions are recorded in the dataset metadata. Where metadata fields differ between generations, the runtime uses the validated top-level parameter entries and supported functional fields rather than inferring undocumented values.
...
tau_j is an upstream node's first activation year. a_ij is its fixed incoming edge weight, not a correlation. The code-defined graph retains equal shares for most targets. Climate retains three 0.25 incoming weights after removal of the unreviewed geopolitics link, totaling 0.75; its other weights were not silently renormalized. Equal weighting, selected topology and the kappa_i mapping are explicit judgments; sources supporting a mechanism do not establish these coefficient values.
...
The network graphic draws each code-defined dependency from upstream source to dependent target, matching the active propagation calculation. The graph has 70 directed links. Thirteen retain a previously documented mechanism; the other 57 remain scenario topology hypotheses requiring review. The unreviewed direct geopolitics-to-climate edge in the old JSON is inactive and absent from the current graph. Twelve links enter the four non-inducible event nodes and are shown as contextual links because they cannot induce those nodes in this cascade rule. The remaining 58 links are eligible for conditional transmission. On selection, blue links enter the selected node and white links leave it; gently moving highlights travel along active eligible links without changing their solid/dashed evidence styling. Motion indicates direction only, not physical speed or strength, and is disabled for reduced-motion users. A node without an outgoing link is a boundary of the declared graph, not a claim that it has no effects in reality. None of these counts measures causal strength, event probability or a validated global failure threshold.
